...
def recognize_data(image_path):
    ...
    match_FIO = re.search(r'RUS(?P<Фамилия>[A-Z0-9]+)<<{0,1}(?P<Имя>[A-Z0-9]+)<(?P<Отчество>[A-Z0-9]+)<<<', full_text_FIO)
    ...
    if match_FIO:
        ...
        data["Серия_и_номер_паспорта"] = full_text_passport
        
        # Сохранение данных в JSON-файл
        save_to_json(data)
        print(f"Данные сохранены в файл output.json")
    else:
    ...
    for i in range(len(recognized_texts) - 2):
        ...
        if re.match(r"^\d{3}[А-Яа-я]{0,1}$", train_candidate) and \
           ...
           re.match(r"^\d{3}$", recognized_texts[i+2]):
            ...
            data["Поезд"] = train_candidate
            data["Вагон"] = recognized_texts[i+1]
            data["Место"] = recognized_texts[i+2]
            break
...
if __name__ == "__main__":
    file_path = "biletik.pdf"
    output_folder = 'src/img/'

    if is_image_or_pdf(file_path):
        pdf_to_jpg(file_path, output_folder)

        if contains_symbols(output_folder + "page_1.jpg"):
            # Паспорт
            recognize_data(output_folder + "page_1.jpg")
        else:
            # Билет
            recognize_text_with_easyocr(output_folder + "page_1.jpg")
    else:
